...
"""PyInstaller spec for the bundled `cognios-server` (desktop sidecar).
...
archive to a temp dir on EVERY launch — 6-7s of "Starting cogniwork…" splash (measured; the
actual Python import is ~0.5s). The wrinkles handled here:
  - aisuite is a regular pip dependency (git-pinned in pyproject.toml); collect cogniwork +
...
# is stripped, not just disabled. Self-builders opt in with COGNIWORK_EXPERIMENTAL=1; the
# loader in cogniwork/connectors/descriptors.py treats the missing package as a no-op.
INCLUDE_EXPERIMENTAL = os.environ.get("COGNIWORK_EXPERIMENTAL") == "1"

hiddenimports = []
datas = []
binaries = []

for pkg in ("cogniwork", "aisuite", "mcp", "ddgs", "croniter", "docstring_parser"):
    hiddenimports += collect_submodules(pkg)

if not INCLUDE_EXPERIMENTAL:
    hiddenimports = [
        m for m in hiddenimports if not m.startswith("cogniwork.connectors.experimental")
    ]
...
a = Analysis(
    ...
    + ([] if INCLUDE_EXPERIMENTAL else ["cogniwork.connectors.experimental"]),
    noarchive=False,
)
pyz = PYZ(a.pure)
exe = EXE(
...
# Onedir: dist/cognios-server/{cognios-server[.exe], _internal/}. The build scripts stage
