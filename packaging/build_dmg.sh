...
#   3. `tauri build --bundles app` → CogniOS.app (resources are copied in).
...
#   COGNIWORK_EXPERIMENTAL=1 ./build_dmg.sh
set -euo pipefail
...
echo "==> [1/5] PyInstaller: bundling cognios-server ($TRIPLE)"
"$PLATFORM/.venv/bin/pyinstaller" --noconfirm --clean \
  --distpath "$HERE/dist" --workpath "$HERE/build" "$HERE/cognios-server.spec"
...
rm -rf "$GUI/src-tauri/binaries/sidecar" "$GUI/src-tauri/binaries/cognios-server-$TRIPLE"
...
cp -RL "$HERE/dist/cognios-server" "$GUI/src-tauri/binaries/sidecar"
...
chmod +x "$GUI/src-tauri/binaries/sidecar/cognios-server"
...
if [ -n "${APPLE_SIGNING_IDENTITY:-}" ]; then
  ...
  find "$SIDECAR" -type f ! -name "cognios-server" \
  ...
  codesign --force --sign "$APPLE_SIGNING_IDENTITY" --timestamp --options runtime \
    --entitlements "$GUI/src-tauri/entitlements.plist" "$SIDECAR/cognios-server"
fi
