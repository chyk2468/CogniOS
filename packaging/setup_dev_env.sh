...
# cognios-server directly, and the Tauri desktop shell falls back to it when
...
python3 -m venv "$VENV"
# The cogniwork package (server, engine, connectors) + inbound-messaging extras.
...
"$VENV/bin/python" -c 'import aisuite, cogniwork' # fail loudly if the wiring broke
echo "Ready: $VENV"
echo "  server: $VENV/bin/cognios-server --cwd /path/to/your/project --port 8765"
