...
def _package_pip_update_status(
    ...
    if pkg.get("kind") == "system" or not pkg.get("pip"):
        return PackageUpdateStatus(
            False, "Update this system dependency outside CogniOS."
        )
    ...
    if name == "vllm" and binaries.get("vllm") and not dists.get("vllm"):
        return PackageUpdateStatus(
            False,
            "Using a vLLM CLI on PATH without Python package metadata; update it outside CogniOS.",
        )
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "cognios-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f'COGNIOS_USER_SHELL="${{SHELL:-}}"\n'
        f'if [ -n "$COGNIOS_USER_SHELL" ] && [ -x "$COGNIOS_USER_SHELL" ]; then\n'
        f'  COGNIOS_USER_PATH="$("$COGNIOS_USER_SHELL" -ic \'printf "__COGNIOS_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__COGNIOS_PATH__//p\' | tail -n 1 || true)"\n'
        f'  if [ -n "$COGNIOS_USER_PATH" ]; then export PATH="$COGNIOS_USER_PATH:$PATH"; fi\n'
        f"fi\n"
