...
def _verify_session_owner(request: Request, session_id: str, session_manager=None):
    """Verify the current user owns the session, honoring single-user modes and admin privileges.

    Authenticated requests must match the stored DB or in-memory owner, unless the caller
    is an admin (admins can manage and delete sessions across all users). When auth is disabled
    and no user is present, treat the app as single-user mode: verify that the session exists.
    """
    user = effective_user(request)
    if not user and not _auth_disabled():
        raise HTTPException(401, "Authentication required")
    app = getattr(request, "app", None)
    auth_manager = getattr(app.state, "auth_manager", None) if app and hasattr(app, "state") else None
    ...
    try:
        ...
        db.close()
    if row is not None:
        if user and row.owner != user and not is_admin:
            raise HTTPException(404, f"Session {session_id} not found")
        return
    # No DB row — allow the caller to act on an in-memory ghost they own (or if admin).
    if session_manager is not None:
        ghost = getattr(session_manager, "sessions", {}).get(session_id)
        if ghost is not None and (not user or getattr(ghost, "owner", None) == user or is_admin):
            return
    raise HTTPException(404, f"Session {session_id} not found")
