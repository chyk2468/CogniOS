...
def setup_history_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    async def delete_messages(request: Request, session_id: str):
        ...
        try:
            ...
            try:
                if msg_ids:
                    # New ID-based delete
                    msg_ids_str = {str(x) for x in msg_ids if x is not None}
                    deleted = 0
                    for mid in msg_ids:
                        db_msg = db.query(DbChatMessage).filter(
                            (DbChatMessage.id == str(mid)) | (DbChatMessage.id == mid),
                    ...
                    session.history = [m for m in session.history if _get_db_id(m) not in msg_ids_str]
                elif indices:
    ...
    async def edit_message(request: Request, session_id: str):
        ...
        try:
            ...
            try:
                db_msg = db.query(DbChatMessage).filter(
                    (DbChatMessage.id == str(msg_id)) | (DbChatMessage.id == msg_id),
                    DbChatMessage.session_id == session_id,
                ).first()
                if not db_msg:
                ...
                for hmsg in session.history:
                    ...
                    if isinstance(hmeta, dict) and str(hmeta.get('_db_id')) == msg_id_str:
