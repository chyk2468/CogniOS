...
def _friendly_email_auth_error(protocol: str, host: str, error: object) -> str:
    ...
    if microsoft_basic_auth_failure:
        return (
            ...
            "Outlook/Office 365 IMAP/SMTP in most accounts. CogniOS "
...
# Override at deploy time via COGNIOS_MAIL_ATTACHMENTS_DIR. Defaults to a
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "cognios_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN cognios_kind TEXT")
...
_IMAP_TIMEOUT_SECONDS = _coerce_imap_timeout_seconds(os.environ.get("COGNIOS_IMAP_TIMEOUT_SECONDS"))
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for CogniOS-generated mail (e.g. reminder, scheduled).
    cognios_kind: Optional[str] = None
