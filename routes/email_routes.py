...
COGNIOS_MAIL_ORIGIN = "cognios-ui"
...
def _apply_cognios_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-CogniOS-Origin"] = COGNIOS_MAIL_ORIGIN
    if kind:
        msg["X-CogniOS-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-CogniOS-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _fixture_email_record(row: dict, uid_num: int, owner: str) -> dict:
        ...
        return {
            "uid": uid,
            "message_id": f"<fixture-email-{uid}-{owner_key}@fixtures.cognios.local>",
            "subject": subject,
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the CogniOS marker header, but include the subject
                # fallback too. The fallback uses a distinct CogniOS prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-CogniOS-Kind "reminder" SUBJECT "Reminder (CogniOS):"{from_clause})',
                )
    ...
    @router.delete("/cognios/reminders")
    async def delete_cognios_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as CogniOS reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-CogniOS-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (CogniOS):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (CogniOS):")})'))
                            # Legacy reminders created before the CogniOS
            ...
            logger.error(f"delete_cognios_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    def _load_cognios_attachment_source(db, kind: str, item_id: str, owner: str):
    ...
    @router.post("/compose-from-cognios")
    async def compose_from_cognios(data: dict, owner: str = Depends(require_owner)):
        """Stage an CogniOS document or gallery image as a compose upload."""
        ...
        try:
            ...
            db = _SL()
            try:
                src = _load_cognios_attachment_source(db, kind, item_id, owner)
                if "path" in src:
            ...
            logger.error(f"Failed to stage CogniOS attachment {kind}/{item_id}: {e}")
            return {"success": False, "error": "Mail operation failed"}

    @router.post("/compose-from-cognios-zip")
    async def compose_from_cognios_zip(data: dict, owner: str = Depends(require_owner)):
        """Stage several CogniOS documents/gallery images as one zip attachment."""
        ...
        try:
            ...
            try:
                ...
                return _stage_compose_bytes("cognios-attachments.zip", content)
            finally:
                db.close()
        except HTTPException:
            raise
        except Exception as e:
            logger.error(f"Failed to stage CogniOS zip attachment: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", cognios_kind=None, cognios_ref=None,
    ):
        ...
        _apply_cognios_headers(outer, cognios_kind or "scheduled", cognios_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, cognios_kind, owner)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("cognios_kind") or "scheduled",
                owner or "",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="cognios.local")
        ...
        if req.cognios_kind:
            _apply_cognios_headers(outer, req.cognios_kind)
