...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When CogniOS runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends CogniOS' own interpreter directory to PATH. If CogniOS itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[cognios] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'COGNIOS_USER_SHELL="${SHELL:-}"',
        'if [ -n "$COGNIOS_USER_SHELL" ] && [ -x "$COGNIOS_USER_SHELL" ]; then',
        '  COGNIOS_USER_PATH="$("$COGNIOS_USER_SHELL" -ic \'printf "__COGNIOS_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__COGNIOS_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$COGNIOS_USER_PATH" ]; then export PATH="$COGNIOS_USER_PATH:$PATH"; fi',
        'fi',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('COGNIOS_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by CogniOS.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$COGNIOS_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $COGNIOS_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$COGNIOS_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for CogniOS-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('COGNIOS_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$COGNIOS_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  COGNIOS_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[cognios] vLLM CLI: $COGNIOS_VLLM_BIN"')
    runner_lines.append('  COGNIOS_VLLM_VERSION="$("$COGNIOS_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$COGNIOS_VLLM_VERSION" ]; then echo "[cognios] vLLM version: $COGNIOS_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('COGNIOS_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $COGNIOS_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $COGNIOS_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $COGNIOS_CMD_EXIT ==="')
        runner_lines.append('exit "$COGNIOS_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _cognios_have_prebuilt=1
    ...
    runner_lines.append('    _cognios_have_prebuilt=""')
    runner_lines.append('    _cognios_arch="$(uname -m)"')
    runner_lines.append('    _cognios_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_cognios_arch" = "x86_64" ]; then')
    runner_lines.append('      _cognios_pat=""')
    runner_lines.append('      _cognios_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _cognios_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _cognios_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _cognios_has_nv_inline; then')
    runner_lines.append('        _cognios_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _cognios_has_vkdev_inline && _cognios_has_vk_inline; then')
    runner_lines.append('        _cognios_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _cognios_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _cognios_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_cognios_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_cognios_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[cognios] Found prebuilt llama-server: $_cognios_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/cognios/llama-cpp-prebuilt" && cd "$HOME/.cache/cognios/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_cognios_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _cognios_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_cognios_extracted" ]; then')
    runner_lines.append('          chmod +x "$_cognios_extracted"')
    runner_lines.append('          ln -sf "$_cognios_extracted" ~/bin/llama-server')
    runner_lines.append('          _cognios_libdir="$(dirname "$_cognios_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_cognios_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/cognios-llama-cpp-env')
    runner_lines.append('          _cognios_have_prebuilt=1')
    runner_lines.append('          echo "[cognios] Prebuilt llama-server installed at $_cognios_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_cognios_have_prebuilt" ] && echo "[cognios] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_cognios_prebuilt_url" ]; then')
    runner_lines.append('      echo "[cognios] No matching prebuilt llama-server for this host (arch=$_cognios_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_cognios_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _cognios_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _cognios_has_nvidia_hw; then')
    ...
    runner_lines.append('    _cognios_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[cognios] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[cognios] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[cognios] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[cognios] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _cognios_apt_bootstrap')
    runner_lines.append('    _cognios_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _cognios_missing_build_deps="$_cognios_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _cognios_missing_build_deps="$_cognios_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _cognios_missing_build_deps="$_cognios_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_cognios_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_cognios_missing_build_deps"')
    ...
    runner_lines.append('    _cognios_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _cognios_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[cognios] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _cognios_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _cognios_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _cognios_has_cudart; then')
    runner_lines.append('        echo "[cognios] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[cognios] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[cognios]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[cognios]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _cognios_has_vulkan_device && _cognios_has_vulkan; then')
    runner_lines.append('      echo "[cognios] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[cognios] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[cognios]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[cognios]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _cognios_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[cognios] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[cognios] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[cognios] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\cognios-sessions"
