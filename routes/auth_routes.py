...
SESSION_COOKIE = "cognios_session"
...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/cognios`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/cognios/cognios` (which ntfy 404s on). ntfy itself
            ...
            headers = {
                "Title": "CogniOS connectivity test",
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from CogniOS. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
