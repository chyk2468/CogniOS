                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in CogniOS (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in CogniOS" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "cognios_kind" if "cognios_kind" in cols else "'scheduled' AS cognios_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `cognios-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the COGNIOS_INPROCESS_POLLERS=0 guidance) -
                ...
                cognios_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-CogniOS-Origin"] = "cognios-ui"
                outer["X-CogniOS-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", cognios_kind or "scheduled")[:64]
                outer["X-CogniOS-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `cognios-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `COGNIOS_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `cognios-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("COGNIOS_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `COGNIOS_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (COGNIOS_INPROCESS_POLLERS=0); "
            "drive `cognios-mail poll-scheduled` externally."
        )
        return
    import asyncio
