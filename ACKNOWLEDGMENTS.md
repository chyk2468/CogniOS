...
CogniOS stands on the shoulders of a lot of open-source work. This file
...
- **[Tongyi DeepResearch](https://github.com/Alibaba-NLP/DeepResearch)** by
  **Alibaba-NLP / Tongyi Lab** — the multi-step deep-research agent pipeline.
  Copyright © Alibaba-NLP / Tongyi Lab. **Apache-2.0.** Adapted for CogniOS's
...
and run alongside CogniOS on `docker compose up`. They are not modified —
just composed.
...
CogniOS talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  CogniOS uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of CogniOS's code was written *with* AI models, not just by a human.
