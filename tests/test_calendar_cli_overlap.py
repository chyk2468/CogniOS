"""Regression: `cognios-calendar list` must select events that OVERLAP the
...
`cognios-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "cognios-calendar"
    loader = importlib.machinery.SourceFileLoader("cognios_calendar_cli", str(path))
