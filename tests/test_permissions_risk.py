...
from cogniwork.permissions import Mode, PermissionEngine
from cogniwork.risk import RiskClass, classify, is_consequential
...
def test_shell_commands_not_auto_allowed_by_default(tmp_path):
    ...
    from cogniwork.config import DEFAULT_ALLOWED_COMMANDS
    ...
    for cmd in (
        ...
        "cat ~/.config/cogniwork/secrets.json",
        "echo $OPENAI_API_KEY",
        "git status",
    ):
