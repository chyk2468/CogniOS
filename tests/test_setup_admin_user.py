...
def _load_setup_module():
    spec = importlib.util.spec_from_file_location("cognios_setup_under_test", Path("setup.py"))
...
def test_create_default_admin_normalizes_env_username(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("COGNIOS_ADMIN_USER", " AdminUser ")
    monkeypatch.setenv("COGNIOS_ADMIN_PASSWORD", "temporary-password")
...
def test_main_loads_admin_password_from_env_file(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("COGNIOS_ADMIN_USER", raising=False)
    monkeypatch.delenv("COGNIOS_ADMIN_PASSWORD", raising=False)
    (tmp_path / ".env").write_text(
        "COGNIOS_ADMIN_USER=presetuser\nCOGNIOS_ADMIN_PASSWORD=fromenvfile12345\n",
        encoding="utf-8-sig",
    )
    ...
    monkeypatch.setenv("COGNIOS_SKIP_ADMIN_PROMPT", "1")
    ...
    try:
        ...
        # don't inherit them.
        os.environ.pop("COGNIOS_ADMIN_USER", None)
        os.environ.pop("COGNIOS_ADMIN_PASSWORD", None)
