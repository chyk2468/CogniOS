...
def test_inbox_approver_allow(tmp_path):
    async def run():
        store = InboxStore(tmp_path / "inbox.json")
        from cogniwork.engine import ApprovalOutcome, PermissionRequest
...
def test_args_preview():
    from cogniwork.inbox import args_preview
...
def test_approval_body_includes_tool_args():
    from cogniwork.engine import PermissionRequest
    from cogniwork.server.manager import _approval_body
