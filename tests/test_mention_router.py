...
per-thread cogniwork session spawns (visible in the sidebar, thread-scoped standing
send_message grant); follow-up tags in the same thread steer the same session. A
channel with a user-connected (subscribed) cogniwork overrides the router — it must
...
from cogniwork.connectors.adapters import slack_event_to_event
from cogniwork.connectors.base import MessageEvent, SessionSource
from cogniwork.conversations import ConversationStore
...
def test_subscribed_cogniwork_overrides_router(tmp_path, monkeypatch):
    mgr = _mgr(tmp_path)
    captured = _capture_deliveries(mgr, monkeypatch)
    ...
    # Delivered to the connected cogniwork with must-respond framing + the thread target…
