...
model actually reads the image (`COGNIWORK_LIVE_VISION=1`, key read from the SecretStore).
"""
...
from cogniwork.attachments import build_user_content, content_to_text
...
def test_list_content_message_persists_and_titles(tmp_path):
    from cogniwork.conversations import ConversationStore, title_from
...
@pytest.mark.skipif(
    os.environ.get("COGNIWORK_LIVE_VISION") != "1",
    reason="opt-in: real OpenAI vision call (set COGNIWORK_LIVE_VISION=1)",
)
...
def test_pdf_attachment_invalid_or_oversized_skipped():
    from cogniwork.attachments import MAX_PDF_CHARS
