...
from cogniwork.mcp import build_callables, load_mcp_servers, tool_name
from cogniwork.mcp.config import MCPServerDef
...
def test_untrusted_workspace_mcp_ignored(tmp_path, monkeypatch):
    """#213: a cloned repo's `.cogniwork/mcp.json` must not load until trust."""
    monkeypatch.setenv("COGNIWORK_STATE_DIR", str(tmp_path / "state"))
    _write_json(
        tmp_path / "state" / "mcp.json",
        {
...
def test_schema_and_metadata():
    ...
    schema = fn.__cogniwork_schema__["function"]
