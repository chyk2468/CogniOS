#!/bin/bash
# CogniOS — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything CogniOS needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine CogniOS
...
# Shell overrides (COGNIOS_PORT / COGNIOS_HOST) take top priority, then .env
# values (APP_PORT / APP_BIND), then built-in defaults.
PORT="${COGNIOS_PORT:-${APP_PORT:-7860}}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
HOST="${COGNIOS_HOST:-${APP_BIND:-127.0.0.1}}" # Set APP_BIND=0.0.0.0 in .env for LAN/Tailscale access.
PROBE_HOST="$HOST"
...
echo "▶ CogniOS quick start for macOS"
...
if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
    ...
    echo "    COGNIOS_PORT=7900 ./start-macos.sh"
    exit 1
fi
...
echo "▶ Preparing CogniOS…"
COGNIOS_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
if [ "$MACHINE_ARCH" = "arm64" ]; then
    if command -v apfel >/dev/null 2>&1; then
        APFEL_LOG="${TMPDIR:-/tmp}/cognios-apfel.log"
...
if (exec 3<>"/dev/tcp/127.0.0.1/$CHROMA_PORT") 2>/dev/null; then
    ...
    CHROMA_LOG="${TMPDIR:-/tmp}/cognios-chromadb.log"
...
# COGNIOS_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$COGNIOS_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
    (
        for _ in $(seq 1 90); do
            if (exec 3<>"/dev/tcp/$PROBE_HOST/$PORT") 2>/dev/null; then
                printf '\n'
                printf '  ┌────────────────────────────────────────────┐\n'
                printf '  │  ✓ CogniOS is ready — opening your browser  │\n'
                printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting CogniOS — it will open in your browser at $URL"
