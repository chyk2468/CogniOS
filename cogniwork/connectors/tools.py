...
def _parse_or_coerce(target: str) -> tuple[str, str, str | None]:
    """parse_target, but a BARE channel name ('all-cognios', '#general') coerces to
...
def _resolve_slack_channel(
    ...
    """'#all-cognios' (a NAME the user said) → the team-qualified chat_id, via the
    ...
    if not c.get("is_member"):
        return None, (
            f"found #{query}, but the bot isn't a member — invite @CogniOS to #{query} "
            "in Slack, then retry"
        )
    return chat_id, None
...
def make_send_message_tool(
    ...
    *,
    ...
    send_message.__cogniwork_schema__ = _SCHEMA
    return send_message
...
def make_send_file_tool(
    ...
    *,
    ...
    send_file.__cogniwork_schema__ = _FILE_SCHEMA
    return send_file
