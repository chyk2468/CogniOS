"""Playwright-backed browser automation tools for CogniWork.
...
class _BrowserController:
    def __init__(self) -> None:
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="cogniwork-browser"
        )
    ...
    def browser_screenshot(path: str = "") -> dict[str, Any]:
        def run(page):
            out = (
                ...
                if path
                else Path(tempfile.gettempdir()) / "cogniwork-browser-screenshot.png"
            )
            out = out.resolve()
