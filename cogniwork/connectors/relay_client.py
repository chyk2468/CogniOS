...
- **Managed relay** (`SlackRelayAdapter`, here): "Add to Slack" OAuth, no tokens
  typed, *many* workspaces, events pushed from CogniOS Cloud over one
...
logger = logging.getLogger("cogniwork.connectors")
...
class SlackRelayAdapter(BasePlatformAdapter):
    ...
    async def _dispatch_slack_event(self, team_id: str, event: dict) -> None:
        ...
        # mirroring the Socket-Mode adapter — so cards read "@CogniOS"/"Rohit"/"#ocw-test"
