"""Launch the server with uvicorn. Used by the desktop GUI sidecar and `cognios-server`."""
...
def _exit_when_orphaned() -> None:
    """When launched as a desktop sidecar (`COGNIWORK_EXIT_WITH_PARENT=1`), exit if the parent
    process dies — even on an abrupt kill (e.g. the Tauri dev watcher restarting the app, or a
    crash) that skips the shell's graceful child-kill. Standalone `cognios-server` runs are
    unaffected.

    The GUI passes its own PID in `COGNIWORK_PARENT_PID`. Watching that explicit PID (not
    ...
    *grandchild* of the GUI — the bootloader sits in between, so getppid() points at the
    ...
    """
    if os.environ.get("COGNIWORK_EXIT_WITH_PARENT") != "1" and os.environ.get("COWORKER_EXIT_WITH_PARENT") != "1":
        return
    import threading

    try:
        parent = int(os.environ.get("COGNIWORK_PARENT_PID") or os.environ.get("COWORKER_PARENT_PID") or 0)
    except ValueError:
        parent = 0
...
def _ensure_api_token(port: int, no_auth: bool = False) -> Path | None:
    ...
    if no_auth:
        ...
        return None
    token = os.environ.get("COGNIWORK_API_TOKEN") or os.environ.get("COWORKER_API_TOKEN")
    if token:
        os.environ["COGNIWORK_API_TOKEN"] = token
...
def main(argv=None) -> None:
    ...
    parser = argparse.ArgumentParser(prog="cognios-server")
    ...
    parser.add_argument("--no-auth", action="store_true", help="disable API token auth for dev")
    ...
    os.environ["COGNIWORK_PORT"] = str(args.port)
    generated_token_path = _ensure_api_token(args.port, no_auth=args.no_auth)
    try:
        import uvicorn
