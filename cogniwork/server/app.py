...
def _browser_page(
    ...
    return (
        ...
        f"<title>{_html.escape(title)} — CogniOS</title><style>"
        ...
        "</style></head><body>"
        '<div class="card"><div class="mark"><i></i>CogniOS</div>'
        f"{icon}<h1>{_html.escape(title)}</h1><p>{_html.escape(detail)}</p>{err}</div>"
        '<div class="foot">Served locally by CogniOS on your Mac</div>'
        "</body></html>"
    )
...
_CONNECT_FAILED_DETAIL = (
    ...
    "Close this tab and try again from CogniOS."
)
...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        try:
            ...
            if live:
                print(f"[cogniwork] messaging gateway live: {', '.join(live)}")
    ...
    app = FastAPI(title="cogniwork", version="0.0.0", lifespan=lifespan)
    api_token = os.environ.get("COGNIWORK_API_TOKEN") or os.environ.get("COWORKER_API_TOKEN", "")
    auth_store = AuthStore(_state_dir() / "cogniwork.db")
    ...
    def _request_authenticated(request: Request) -> bool:
        provided = request.headers.get("x-cognios-token") or request.headers.get("x-openworker-token", "")
        return bool(
            api_token
            and provided
    ...
    async def require_sidecar_token(request: Request, call_next):
        ...
        return JSONResponse(
            {"error": "missing or invalid CogniOS sidecar token"},
            status_code=401,
        )
    ...
    app.include_router(create_auth_router(auth_store, auth_limiter, manager))
    ...
    async def transcribe_voice(request: Request) -> dict[str, Any]:
        """Transcribes recorded audio using local whisper.cpp with ggml-large-v3-turbo-q5_0."""
        content_type = request.headers.get("content-type", "")
        audio_bytes = b""
        if "application/json" in content_type:
            try:
                body = await request.json()
                b64_str = body.get("audio_b64", "")
                if b64_str:
        ...
        if not audio_bytes:
            return {"ok": False, "error": "No audio data received.", "text": ""}

        root_dir = Path(__file__).resolve().parent.parent.parent
        possible_bins = [
            root_dir / "stt" / "whisper.cpp" / "build" / "bin" / "whisper-cli.exe",
            root_dir / "stt" / "whisper.cpp" / "build" / "bin" / "whisper-cli",
            root_dir / "stt" / "whisper.cpp" / "build" / "bin" / "main.exe",
            root_dir / "stt" / "whisper.cpp" / "build" / "bin" / "main",
        ]
        possible_models = [
            Path(os.environ.get("COGNIWORK_WHISPER_MODEL_PATH", "")),
            root_dir / "stt" / "whisper.cpp" / "models" / "ggml-large-v3-turbo-q5_0.bin",
            _state_dir() / "models" / "ggml-large-v3-turbo-q5_0.bin",
        ]

        cli_path = next((p for p in possible_bins if p.is_file()), None)
        model_path = next((p for p in possible_models if p.is_file()), None)

        if not cli_path or not model_path:
            return {"ok": False, "error": "Local whisper model or executable not found.", "text": ""}
        ...
        wav_path = tmp_audio_path + ".converted.wav"
        input_file = tmp_audio_path
        try:
            conv = await asyncio.to_thread(
                subprocess.run,
                ["ffmpeg", "-y", "-i", tmp_audio_path, "-ar", "16000", "-ac", "1", "-c:a", "pcm_s16le", wav_path],
                capture_output=True,
                timeout=15,
            )
        ...
        try:
            ...
            lines = [
                ...
                if line.strip()
                and not line.startswith("read_audio_data:")
                and not line.startswith("whisper_")
                and not line.startswith("main:")
                and not line.startswith("system_info:")
            ]
            transcript = " ".join(lines).strip()
            transcript = re.sub(r"\[\d{2}:\d{2}:\d{2}\.\d{3}\s*-->\s*\d{2}:\d{2}:\d{2}\.\d{3}\]\s*", "", transcript).strip()
    ...
    def subscriptions() -> dict[str, Any]:
        ...
        for sub in manager.subscriptions.all():
            ...
            routing = manager._routing_targets(sub.session_id, agent or "cogniwork")
            out.append(
                {
    ...
    async def mcp_oauth_callback(
        ...
        if error:
            return HTMLResponse(
                _browser_page(
                    "Sign-in failed",
                    "The service reported an error. Return to CogniOS and try again.",
                    ok=False,
                    error=error,
                ),
                status_code=400,
        ...
        if not code or not mcp_oauth.deliver_callback(code, state or None):
            return HTMLResponse(
                _browser_page(
                    ...
                    "The sign-in may have timed out. Return to CogniOS and start it again.",
                    ok=False,
                ),
                status_code=400,
            )
        return HTMLResponse(
            _browser_page(
                "Connected",
                "Sign-in complete. You can close this tab and return to CogniOS.",
                ok=True,
            )
        )
    ...
    async def gcal_account_disconnect(email: str) -> dict[str, Any]:
        """Drop ONE Google Calendar account;
    ...
    if os.environ.get("COGNIWORK_DEBUG_INJECT") == "1":
    ...
    async def ws_session(ws: WebSocket, session_id: str) -> None:
        ...
        if not _origin_allowed(ws.headers.get("origin")):
            await ws.close(code=1008)
            return
        await ws.accept(subprotocol="cognios" if api_token else None)
