...
logger = logging.getLogger("cogniwork.manager")
...
class SessionManager:
    def __init__(
        ...
        *,
        ...
        if data_dir is not None:
            ...
            base = Path(self.default_workspace) / ".cogniwork"
        else:
            base = state_dir()
        base.mkdir(parents=True, exist_ok=True)

        self.memory_store: MemoryStore = SQLiteMemoryStore(base / "cogniwork.db")
        ...
        self.audit_store = AuditStore(base / "cogniwork.db")
    ...
    def _mcp_workspace_trusted(self, workspace: str | Path | None) -> bool:
        """Whether workspace `.cogniwork/mcp.json` may be loaded (#213).
    ...
    def get_engine(
        ...
        *,
        ...
        if ag.needs_workspace and (not ws or not Path(ws).is_dir()):
            # Knowledge surfaces (CogniWork, Ops, …) start "orphan": no folder picked →
    ...
    def set_persona_enabled(self, persona_id: str, enabled: bool) -> dict[str, Any]:
        ...
        non-internal) sessions — disable means "put this cogniwork and its history away", so
    ...
    def session_connections_view(
        ...
        the owner's 2026-07-03 finding (a fresh Project Manager session rendered cogniwork's view).
        """
        persona = self._persona_of(session_id, persona_id)
        entry = self.personas.get(persona)
        manifest = entry.manifest if entry else None
    ...
    def list_artifacts(self, session_id: str) -> list[dict[str, Any]]:
        ...
        artifacts_sub = ws_path / "Artifacts"
        root = artifacts_sub if artifacts_sub.is_dir() else ws_path
        if not root.is_dir():
            return []
        ...
        # TCC prompt ("CogniOS would like to access data from other apps") on every turn.
    ...
    def _artifact_target(
        ...
        base_root = Path(workspace).expanduser().resolve()
        artifacts_root = (base_root / "Artifacts").resolve()
        
        target = (artifacts_root / path).expanduser().resolve()
        
        if (allow_dir and target.is_dir()) or target.is_file():
            try:
        ...
        fallback_target = (base_root / path).expanduser().resolve()
        try:
            fallback_target.relative_to(base_root)
        except ValueError:
            return None, "path escapes workspace"
            
        if allow_dir and fallback_target.is_dir():
            return fallback_target, None
        if not fallback_target.is_file():
            return None, (
    ...
    def pick_native_folder(self) -> dict[str, Any]:
        ...
        if sys.platform == "darwin":
            cmd = [
                ...
                "-e",
                'POSIX path of (choose folder with prompt "Give the cogniwork access to a folder")',
            ]
        elif sys.platform == "win32":
            ...
            ps = (
                ...
                "$f.Description = 'Give the cogniwork access to a folder'; "
    ...
    def get_settings(self) -> dict[str, Any]:
        ...
        return {
            ...
            # hardcoded POSIX one (Windows -> %APPDATA%\cogniwork, macOS/Linux -> ~/.config).
    ...
    def _surfaces(self) -> dict[str, bool]:
        """Which session surfaces are shown in the sidebar. CogniWork is always on; Chat and Code
        are opt-in (default off) so a new user sees CogniWork only."""
        return {
            "cogniwork": True,
    ...
    def set_surfaces(
        ...
        """Toggle Chat/Code visibility (CogniWork is always shown). Persisted in prefs."""
        if chat is not None:
    ...
    def set_scratch_base(self, path: str) -> dict[str, Any]:
        """Set + persist the common area where each CogniWork conversation's scratch directory is
        created (default ~/CogniOS). The raw value is stored so the UI shows it as entered;
    ...
    async def refresh_gateway(self) -> list[str]:
        ...
        print(f"[cogniwork] messaging gateway reloaded: {', '.join(started) or 'none'}")
        return started
    ...
    def _note_person(
        ...
        chips above all -- can show who a user ID actually is. Best-effort, newest name wins.
        """
    ...
    async def _dispatch_inbound(self, event) -> None:
        ...
        if src.chat_type in ("channel", "group"):
            ...
            # the per-thread cogniwork session.
    ...
    async def _route_mention(self, event, ms: MessageSource, subs) -> None:
        """@CogniOS tagged in a channel. A subscribed (user-connected) cogniwork owns the channel
        and must answer; otherwise the per-thread cogniwork session handles it — spawned on the
        ...
        if subs:
            # The user connected a cogniwork to this channel — it answers tags; no spawn.
            msg = (
    ...
    async def _spawn_mention_session(
        ...
        """First tag in a thread: a NEW visible cogniwork session that owns the thread. Its
    ...
    # -- session roots (orphan CogniWork: scratch + added folders) ------------------
            ...
            if self.session_store.load(session_id) is None:
                self.session_store.save(
                    SessionRecord(
                        ...
                        messages=[],
                        agent="cogniwork",  # folder access is a CogniWork affordance
                    )
                )
