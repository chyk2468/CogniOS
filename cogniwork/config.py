...
Workspace: <workspace>/.cogniwork/config.toml   (overrides global)
...
class Config:
    ...
    # CogniOS Cloud (sign-in + managed connectors). Config, never constants:
    # dev/staging/BYO-VPC deployments point these at their own instances.
    cloud_base_url: str = "https://api.cognios.com"
...
def workspace_allowed_commands(workspace: str | Path) -> list[str]:
    ...
    path = Path(workspace).expanduser() / ".cogniwork" / "config.toml"
...
def load_config(
    ...
    *,
    ...
    if workspace:
        w = Path(workspace).expanduser() / ".cogniwork" / "config.toml"
        if w.is_file():
