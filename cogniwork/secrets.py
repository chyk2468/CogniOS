...
`${ENV_VAR}` references resolved at read time from the process env / `~/.config/cogniwork/.env`.
...
def state_dir() -> Path:
    """Where cogniwork keeps its state — the one cross-platform source of truth.

    Resolution order:
    1. `$COGNIWORK_STATE_DIR` (or legacy `$COWORKER_STATE_DIR`) — explicit override.
    2. Windows: `%APPDATA%\\cogniwork`, falling back to legacy `%APPDATA%\\coworker`.
    3. macOS / Linux: `~/.config/cogniwork`, falling back to legacy `~/.config/coworker`.
    """
    base = os.environ.get("COGNIWORK_STATE_DIR") or os.environ.get("COWORKER_STATE_DIR")
    if base:
        return Path(base).expanduser()
    if sys.platform == "win32":
        appdata = os.environ.get("APPDATA")
        if appdata:
            new_dir = Path(appdata) / "cogniwork"
            legacy_dir = Path(appdata) / "coworker"
            return legacy_dir if legacy_dir.is_dir() and not new_dir.is_dir() else new_dir
    config = Path.home() / ".config"
    new_dir = config / "cogniwork"
    legacy_dir = config / "coworker"
    return legacy_dir if legacy_dir.is_dir() and not new_dir.is_dir() else new_dir
