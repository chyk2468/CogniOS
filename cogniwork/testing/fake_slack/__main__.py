"""Standalone FakeSlack runner — drive the live dev app against a fake Slack.
    ...
    python -m cogniwork.testing.fake_slack --port 8910
...
``cognios-server`` and connecting Slack with any fake ``xoxb-``/``xapp-`` tokens.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="cogniwork.testing.fake_slack")
    parser.add_argument(
    ...
    print(
        "  # then start cognios-server and connect Slack with any xoxb-/xapp- tokens"
    )
    print()
