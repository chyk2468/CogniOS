...
def _skill_dirs(workspace: Path | None) -> list[Path]:
    ...
    if workspace is not None:
        dirs.append(workspace / ".cogniwork" / "skills")
    return dirs
...
def build_engine(
    *,
    ...
    # The session's directories. Explicit `roots` (orphan CogniWork: scratch + added folders) wins;
    ...
    # Messaging personas (CogniWork / Ops / MyHelper) expose send_message; MyHelper also uses it as
    ...
    if memory_store is not None:
        ...
        instructions = f"{instructions}\n\n{_MEMORY_GUIDANCE}"
        # What the cogniwork KNOWS is fixed at session start (MEMORY-SPEC §7.1): a
    ...
    # directory list (orphan CogniWork can gain folders mid-session; CogniWork/MyHelper only), and the
    ...
    def context_provider() -> str:
        ...
        # knowledge, so it must bite the moment the user flips it. What the cogniwork
