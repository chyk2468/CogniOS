...
SUMMARY_SYSTEM_PROMPT = """You are compacting an AI cogniwork's session history so the cogniwork can continue working in a smaller context. Write a structured summary of the conversation below. It is the cogniwork's ONLY memory of these turns, so preserve everything load-bearing.
...
Rules:
- Do NOT carry full file contents as truth. Note THAT a file was read/edited; the cogniwork re-reads if it needs the content again. Stale memory of a file is worse than no memory.
