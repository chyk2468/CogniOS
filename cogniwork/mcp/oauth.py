...
def redirect_base() -> str:
    ...
    port = os.environ.get("COGNIWORK_PORT") or "8765"
...
class _InteractiveAuthFilter(logging.Filter):
    """Suppress the SDK's noisy 'OAuth flow error' traceback when we deliberately raise."""
    def filter(self, record: logging.LogRecord) -> bool:
        if record.exc_info and isinstance(record.exc_info[1], InteractiveAuthRequired):
            return False
        return True

logging.getLogger("mcp.client.auth.oauth2").addFilter(_InteractiveAuthFilter())
