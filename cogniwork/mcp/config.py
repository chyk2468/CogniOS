...
Global:    ~/.config/cogniwork/mcp.json
Workspace: <workspace>/.cogniwork/mcp.json   (overrides global on name clash,
...
def _config_paths(
    ...
    so an untrusted repo's `.cogniwork/mcp.json` is never read — cloning alone must
    not be enough to define processes that run at session open.
    """
    paths = [global_mcp_path()]
    if workspace and workspace_trusted:
        paths.append(Path(workspace).expanduser() / ".cogniwork" / "mcp.json")
    return paths
