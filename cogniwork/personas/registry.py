...
CogniWork) wrap their existing agent builders (exact prompts preserved), and markdown manifests
...
from ..agents.cogniwork import COGNIWORK_CAPABILITIES, cogniwork_agent
from .manifest import PersonaManifest, load_manifest_file

DEFAULT_PERSONA_ID = "cogniwork"
...
class PersonaRegistry:
    ...
    def _load_builtin(self, builtin_dir: str | Path | None) -> None:
        # Core surfaces keep their exact prompts via the existing builders. CogniWork (the default)
        # leads; Chat is hidden from the picker by default (CogniWork covers quick Q&A) — recoverable
    ...
    def is_enabled(self, persona_id: str) -> bool:
        ...
        # 2026-07-09): a fresh install is CogniWork-only, everything else is opt-in from
    ...
    def default_id(self) -> str:
        # The configured default if it's enabled, else cogniwork if present, else any enabled one.
