#!/usr/bin/env python3
"""CogniOS — first-time setup script.
...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("COGNIOS_ADMIN_USER", "").strip().lower()
        password = os.getenv("COGNIOS_ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — validate before using
            if username in RESERVED_USERNAMES:
                print(f"  [error] COGNIOS_ADMIN_USER '{username}' is a reserved username")
                return "failed"
            if len(password) < PASSWORD_MIN_LENGTH:
                print(f"  [error] COGNIOS_ADMIN_PASSWORD must be at least {PASSWORD_MIN_LENGTH} characters")
                return "failed"
        elif sys.stdin.isatty() and not os.getenv("COGNIOS_SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not os.getenv("COGNIOS_ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not os.getenv("COGNIOS_ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set COGNIOS_ADMIN_PASSWORD to choose your own. **")
        return "created"
...
def main():
    ...
    # Load .env so pre-seeded COGNIOS_ADMIN_USER / COGNIOS_ADMIN_PASSWORD (and
    ...
    if not os.getenv("COGNIOS_SKIP_RUN_HINT"):
