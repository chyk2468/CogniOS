...
def _is_mlx_deepseek_v4_shim_id(model_id: str) -> bool:
    return "/.cache/cognios/mlx-shims/deepseek-v4" in str(model_id or "").lower()
...
def build_headers(api_key: Optional[str], base: str) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        headers.setdefault("HTTP-Referer", "https://github.com/cognios-dev/cognios")
        headers.setdefault("X-OpenRouter-Title", "CogniOS")
