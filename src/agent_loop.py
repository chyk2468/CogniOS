...
Streaming agent loop for cognios-ui.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- CogniOS chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
GENERIC LOOPBACK to allowed CogniOS internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(r"\b(cookbook|serve|serving|served|launch|start|preset|vllm|sglang|llama\.?cpp|ollama|download|downloading|pull|cached models?|running models?|model servers?|models? (?:are )?running|what models?|model picker|gpu box|kierkegaard|cognios|ajax|qwen|gemma|llama|mistral|minimax)\b"):
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] cognios doc minimal memory facts=%s", len(facts))
    return {
        "role": "user",
        "content": (
            "Saved user memory facts from CogniOS Brain. These are the same "
...
def _minimal_cognios_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the CogniOS document LoRA.
    ...
    if stream_create:
        system = (
            "You are CogniOS. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are CogniOS. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, CogniOS will answer Done."
        )
...
def _minimal_cognios_notes_messages(messages: List[Dict]) -> List[Dict]:
    """Tiny prompt path for CogniOS notes LoRAs.

    The finetune is trained to emit CogniOS note tool calls without receiving
    the full tool schema or saved-context wrapper stack.
    """
    latest = _extract_last_user_message(messages)
    system = (
        "You are CogniOS. Handle note, todo, checklist, and reminder requests.\n"
        "You have access to the user's CogniOS notes through manage_notes.\n"
        "For 'what are my notes', 'show my notes', note searches, note creation, todos, checklists, and reminders, use the CogniOS manage_notes tool call format.\n"
...
def _minimal_cognios_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    """Minimal fallback for CogniOS finetunes outside domain-specific paths."""
    latest = _extract_last_user_message(messages)
    system = (
        "You are CogniOS. Answer directly and briefly.\n"
        "Use CogniOS tool-call format only when the user explicitly asks you to take an action.\n"
    ...
    if active_document:
        ...
        if _is_email_doc:
            ...
            doc_ctx = (
                ...
                f'2. Replace ONLY the new reply text above `---------- Previous message ----------`. You may omit the quoted history from your tool output; CogniOS preserves everything from that separator downward automatically.\n'
...
async def stream_agent_loop(
    ...
    _ody_qwen_finetune_model = (model or "").lower().startswith("cognios-qwen3")
    ...
    if _direct_low_signal:
        ...
        direct_messages = (
            _minimal_cognios_general_messages(
                messages,
                include_memory=True,
            )
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        ...
        logger.info("[agent-intent] cognios doc finetune tool clamp=%s", sorted(_relevant_tools))
    elif _ody_notes_finetune_mode and _relevant_tools is not None:
        _relevant_tools = {"manage_notes", "ask_user", "update_plan"}
        logger.info("[agent-intent] cognios notes finetune tool clamp=%s", sorted(_relevant_tools))
    ...
    if _ody_doc_finetune_mode and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_cognios_doc_messages(
            messages,
        ...
        mcp_schemas = []
        logger.info(
            "[agent-intent] cognios doc minimal prompt active active_doc=%s stream_create=%s messages=%s",
        ...
        messages = _minimal_cognios_notes_messages(messages)
        mcp_schemas = []
        logger.info(
            "[agent-intent] cognios notes minimal prompt active messages=%s",
            len(messages),
        )
    elif _ody_qwen_finetune_model and not plan_mode and not approved_plan and not guide_only:
        messages = _minimal_cognios_general_messages(
            messages,
            include_memory=True,
        )
        mcp_schemas = []
        logger.info(
            "[agent-intent] cognios general minimal prompt active include_memory=%s messages=%s",
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] cognios doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] cognios doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _ody_qwen_finetune_model and tool_blocks:
            ...
            if _dropped_memory_lookup:
                logger.info(
                    "[agent-intent] cognios qwen dropped manage_memory lookup; answering from compact memory"
                )
        ...
        if _doc_stream_create_completed:
            ...
            logger.info("[agent] cognios doc stream-create completed after one create_document")
            break
        ...
        if _ody_doc_tool_completed:
            if not full_response.strip() or full_response.strip().startswith("```"):
                full_response = "Done."
                yield 'data: ' + json.dumps({"delta": "Done."}) + '\n\n'
            logger.info("[agent] cognios doc tool completed after one textual tool block")
            break

        if _ody_notes_finetune_mode and _ody_notes_tool_completed:
            logger.info("[agent] cognios notes completed from deterministic tool output")
            break
