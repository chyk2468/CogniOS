...
MCP_DISABLED = os.environ.get("COGNIOS_DISABLE_MCP", "").lower() in ("1", "true", "yes")
...
async def register_builtin_servers(mcp_manager):
    ...
    if MCP_DISABLED:
        logger.info("Built-in MCP servers disabled via COGNIOS_DISABLE_MCP")
        return
        ...
        for server_id, cfg in _BUILTIN_NPX_SERVERS.items():
            ...
            if pkg_spec and not await _is_npx_package_cached(npx_path, pkg_spec):
                logger.warning(
                    ...
                    f"          (run once, then restart CogniOS)\n"
