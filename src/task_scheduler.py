...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = "Queued — waiting for CogniOS to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because CogniOS became active", task.name)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    "Paused because CogniOS became active"
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-CogniOS-Origin"] = "cognios-ui"
            msg["X-CogniOS-Kind"] = "task"
            msg["X-CogniOS-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-CogniOS-Origin": "cognios-ui",
                "X-CogniOS-Kind": "task",
                "X-CogniOS-Ref": str(task.id),
            },
        }
        if recipient:
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = "CogniOS became active") -> int:
        ...
        user opens or uses CogniOS, foreground interaction wins immediately.
