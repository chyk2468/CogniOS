...
def _sanitize_windows_invalid_chars(path_str: str) -> str:
    """Sanitize invalid path characters (especially trailing/mid-path colons) on Windows.

    For example: 'pro:/1.html' or 'pro:1.html' or 'pro:\1.html' -> 'pro/1.html'.
    Leaves legitimate drive letters (e.g. 'C:\...') untouched.
    """
    if not path_str or not isinstance(path_str, str):
        return path_str
    s = path_str.strip()
    # Preserve drive letter at position 0..1 (e.g. C: or D:)
    has_drive = len(s) >= 2 and s[1] == ':' and s[0].isalpha()
    drive_part = s[:2] if has_drive else ""
    rest_part = s[2:] if has_drive else s
    if ":" in rest_part:
        import re
        parts = re.split(r'[/\\]+', rest_part)
        cleaned_parts = [p.rstrip(":") for p in parts]
        rest_part = "/".join(cleaned_parts)
    return drive_part + rest_part
...
def _resolve_tool_path(raw_path: str) -> str:
    ...
    raw_path = _sanitize_windows_invalid_chars(str(raw_path).strip())
...
_EMAIL_MCP_OWNER_ARG = "_cognios_owner"
