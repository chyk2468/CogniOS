...
const _ACTION_RECENTS_KEY = 'cognios-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'cognios-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'CogniOS';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'CogniOS' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
