...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'cognios-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('cognios-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('cognios-compare-synth-' + state._compareMode);
