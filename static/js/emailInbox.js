...
function _openEmailTagFilter(tag) {
  ...
  setTimeout(() => {
    document.dispatchEvent(new CustomEvent('cognios:email-filter-tag', { detail: { tag: normalized } }));
  }, 0);
}
...
let _showEmailTags = localStorage.getItem('cognios.email.showTags') !== '0';

export function init(documentModule) {
  _docModule = documentModule;
  _bindEvents();
  document.addEventListener('cognios:email-tags-toggle', (e) => {
...
function _bindEvents() {
  ...
  // Delay the lightweight unread badge check so opening CogniOS doesn't
...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const lastSeen = parseInt(localStorage.getItem('cognios-email-last-seen-uid') || '0', 10);
  ...
  try {
    ...
    fetch(emailApiUrl('/api/email/unread-state', { folder: 'INBOX' }))
      ...
      .then(data => {
        const maxUid = parseInt(data.max_uid || '0', 10) || 0;
        if (maxUid > 0) {
          localStorage.setItem('cognios-email-last-seen-uid', String(maxUid));
        }
        const dot = document.getElementById('email-unread-dot');
        if (dot) dot.style.display = 'none';
      })
