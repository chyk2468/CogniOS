...
export function initKeyboardShortcuts(modules) {
  ...
  window._cogniosKeybinds = { ..._defaultKeybinds };

  // Load saved keybinds
  fetch('/api/auth/settings', { credentials: 'same-origin' })
    .then(r => r.json())
    .then(s => { if (s.keybinds) window._cogniosKeybinds = { ..._defaultKeybinds, ...s.keybinds }; })
    .catch(() => {});
  ...
  document.addEventListener('keydown', (e) => {
    const kb = window._cogniosKeybinds;
      ...
      uiModule.styledConfirm('Delete this session?', { confirmText: 'Delete', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = 'CogniOS Chat';
