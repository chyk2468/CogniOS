...
function _renderHwVisibilityWarning(sys) {
  ...
  box.querySelector('[data-hw-action="copy"]')?.addEventListener('click', () => {
    ...
    const text = [
      'CogniOS Cookbook hardware diagnostics',
      ...
      'docker compose exec cognios cat /proc/meminfo | head',
      'docker compose exec cognios python -c "from services.hwfit.hardware import detect_system; import json; print(json.dumps(detect_system(fresh=True), indent=2))"',
    ].join('\n');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    // dropdown still showed cognios. The user's selection must only change via
