...
let _sortMode = Storage.get('cognios-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'CogniOS Chat';
...
const FOLDER_STATE_KEY = 'cognios-folder-state';
const FOLDER_ORDER_KEY = 'cognios-folder-order';
...
function createSessionItem(s) {
  ...
  deleteItem.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/session/${s.id}`, { method: 'DELETE' });
      if (!res.ok) {
        const errData = await res.json().catch(() => ({}));
        const msg = (errData.detail && (errData.detail.message || errData.detail)) || 'Failed to delete session';
        if (window.uiModule && window.uiModule.showError) {
          window.uiModule.showError(msg);
        }
      }
    } catch (e) {
      if (window.uiModule && window.uiModule.showError) {
        window.uiModule.showError('Failed to delete session');
      }
    }
    await loadSessions();
  });
...
export async function selectSession(id, { keepSidebar = false, showLoading = true, immediateLoading = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to CogniOS kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'CogniOS Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('cognios-doc-open-' + id) === '1';
...
export function deselectCurrentSession(sid) {
  _deselectCurrentSession(sid);
}

export function removeSessionFromLocalState(sid) {
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('cognios-session-sort', mode);
  else Storage.remove('cognios-session-sort');
  renderSessionList();
}
