...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'cognios.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__cogniosActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'cognios-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'cognios-doc-minimized-' + sessionId;
  ...
  let _cogniosAttachMenu = null;

  function _closeCogniOSAttachMenu() {
    if (_cogniosAttachMenu) {
      _cogniosAttachMenu.remove();
      _cogniosAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_cogniosAttachMenu && !_cogniosAttachMenu.contains(e.target)) _closeCogniOSAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeCogniOSAttachMenu();
  }

  function _positionCogniOSAttachMenu(anchor, menu) {
  ...
  function _cogniosAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageCogniOSAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-cognios`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageCogniOSZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-cognios-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterCogniOSAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachCogniOSItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageCogniOSAttachment(kind, id);
      if (!data) return;
      _afterCogniOSAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeCogniOSAttachMenu();
    } catch (err) {
      console.error('Failed to attach CogniOS item:', err);
      if (uiModule) uiModule.showError('Failed to attach from CogniOS');
    }
  }

  function _selectedCogniOSAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-cognios-attach-row.is-selected') || []);
  }

  function _syncCogniOSAttachSelection(menu) {
    const selected = _selectedCogniOSAttachRows(menu);
    const bar = menu?.querySelector?.('.email-cognios-attach-actions');
    const count = menu?.querySelector?.('.email-cognios-attach-count');
    const attachBtn = menu?.querySelector?.('.email-cognios-attach-selected');
  ...
  async function _attachSelectedCogniOSItems(menu) {
    const rows = _selectedCogniOSAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-cognios-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageCogniOSZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageCogniOSAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterCogniOSAttachmentsAdded(added, zip ? 'cognios-attachments.zip' : undefined);
      _closeCogniOSAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected CogniOS items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from CogniOS');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadCogniOSAttachItems(menu, kind) {
    const list = menu.querySelector('.email-cognios-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-cognios-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-cognios-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncCogniOSAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _cogniosAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-cognios-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-cognios-attach-dot" aria-hidden="true"></span>
            <span class="email-cognios-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-cognios-attach-main">
              <span class="email-cognios-attach-title">${_escHtml(label)}</span>
              <span class="email-cognios-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-cognios-attach-dot" aria-hidden="true"></span>
            <span class="email-cognios-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-cognios-attach-main">
              <span class="email-cognios-attach-title">${_escHtml(label)}</span>
              <span class="email-cognios-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachCogniOSItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncCogniOSAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load CogniOS attach items:', err);
      list.innerHTML = '<div class="email-cognios-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-cognios-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-cognios-attach-local">
      ...
      <div class="email-cognios-attach-tabs">
      ...
      <label class="email-cognios-attach-search-wrap">
        ...
        <input type="search" class="email-cognios-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-cognios-attach-list"></div>
      <div class="email-cognios-attach-actions">
        <span class="email-cognios-attach-count"></span>
        <button type="button" class="email-cognios-attach-selected" disabled>
    ...
    _cogniosAttachMenu = menu;
    _positionCogniOSAttachMenu(anchor, menu);
    menu.querySelector('.email-cognios-attach-local')?.addEventListener('click', () => {
      _closeCogniOSAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadCogniOSAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-cognios-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadCogniOSAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-cognios-attach-selected')?.addEventListener('click', () => _attachSelectedCogniOSItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadCogniOSAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__cogniosActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'cognios:email-ai-reply-context:v1:';
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('cognios-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('cognios-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'cognios-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('cognios-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('cognios-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('cognios-suggestions-' + docId);
      if (!raw) return;
