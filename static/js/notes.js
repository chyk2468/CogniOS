...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('cognios-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'cognios-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'cognios-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'cognios-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'cognios-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'cognios-notes-reminder-dismissed-at';
const NOTES_FIRST_OPEN_HINT_KEY = 'cognios-notes-first-open-hint-v1';
...
function _bringNotesToFront(pane = document.getElementById('notes-pane')) {
  ...
  try {
    window.dispatchEvent(new CustomEvent('cognios:modal-opened', {
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('cognios-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
const _DRAFT_PREFIX = 'cognios-note-draft-';
