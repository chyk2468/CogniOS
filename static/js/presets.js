  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = loadStoredArray('cognios-hidden-presets');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('cognios-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = loadStoredArray('cognios-hidden-presets');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = loadStoredObject('cognios-char-sessions');
      charSessions[sessionId] = charName;
      localStorage.setItem('cognios-char-sessions', JSON.stringify(charSessions));
