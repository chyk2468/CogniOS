  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false && pkg.name !== 'llama_cpp') {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside CogniOS.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('CogniOS app', 'Run inside the CogniOS app itself.', _appDeps),
  ...
  if (dlBtn && dlInput) {
    ...
    const triggerDownload = async () => {
      ...
      if (looksGgufRepo && !pickerInclude) {
        ...
        if (!pickerInclude) {
          uiModule.showToast('Pick a GGUF quant first. CogniOS will not download the whole GGUF repo without an include pattern.');
          return;
        }
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="display:none;">'
       + 'Don\'t see a model? '
       + '<a href="https://github.com/cognios-dev/cognios/discussions/1962" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;position:relative;top:-1px;">'
       + 'Request it →'
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend CogniOS capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install CogniOS keys, choose model directories, and set the default server. Local is this machine.</p>';
...
const SHARED_STATE_LEADER_KEY = 'cognios-cookbook-shared-state-leader';
