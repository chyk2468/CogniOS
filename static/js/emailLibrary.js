...
document.addEventListener('cognios:email-filter-tag', (e) => {
...
try { window.__cogniosGetActiveEmailContext = _getActiveEmailContext; } catch (_) {}
...
window.addEventListener('cognios-reminder-channel-changed', (e) => {
...
function _isChatInteractionBusy() {
  try {
    if (window.__cogniosChatBusy) return true;
    const until = Number(window.__cogniosChatBusyUntil || 0);
...
function _publishActiveAccount() {
  try { window.__cogniosActiveEmailAccount = state._libAccountId || null; } catch (_) {}
...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete CogniOS reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show CogniOS reminder emails">
  ...
  if (tagsToggle) {
    ...
    tagsToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('cognios.email.showTags', state._libShowTags ? '1' : '0');
      tagsToggle.classList.toggle('active', !!state._libShowTags);
      tagsToggle.setAttribute('aria-pressed', String(!!state._libShowTags));
      _renderGrid();
      document.dispatchEvent(new CustomEvent('cognios:email-tags-toggle', { detail: { show: state._libShowTags } }));
    });
  }
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all CogniOS reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/cognios/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
...
const _BUBBLES_DISABLED_KEY = 'cognios.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'cognios.email.summaryCollapsed';
