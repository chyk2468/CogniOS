...
const BADGE_SEEN_KEY = 'cognios-calendar-badge-seen';
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('cognios.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('cognios.cal.detailH', String(px)); } catch {} }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('cognios.cal.detailH'); } catch {}
      };
...
const LS_KEY = 'cognios-calendar-cache';
