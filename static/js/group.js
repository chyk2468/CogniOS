...
const GROUP_STATE_KEY = 'cognios-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('cognios-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('cognios-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
