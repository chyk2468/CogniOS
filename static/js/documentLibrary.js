...
export function initLibrary(config) {
  const rawBase = (config && config.apiBase) || (typeof window !== 'undefined' && window.API_BASE) || '';
  API_BASE        = (rawBase && rawBase !== 'undefined') ? rawBase : '';
  _esc            = config.esc;
...
// ---- Library animation helpers ----
  ...
  export function openLibrary(opts) {
    ...
    async function _toggleChatPreview(card, session) {
      ...
      try {
        ...
        if (deleteBtn) deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          const base = (typeof API_BASE === 'string' && API_BASE && API_BASE !== 'undefined') ? API_BASE : (window.API_BASE || '');
          await fetch(base + '/api/session/' + session.id, { method: 'DELETE', credentials: 'same-origin' });
    ...
    document.getElementById('doclib-chats-bulk-delete').addEventListener('click', async () => {
      ...
      const ids = [..._chatsSelected];
      const results = await Promise.all(
        ids.map(sid => fetch(base + '/api/session/' + sid, { method: 'DELETE', credentials: 'same-origin' })
          .then(r => ({ sid, ok: r.ok }))
          .catch(() => ({ sid, ok: false }))
        )
      );
