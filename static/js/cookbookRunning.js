...
function _winSessionCmd(task, tmuxArgs) {
  const host = _taskRemoteHost(task);
  const sd = host ? '$env:TEMP\\cognios-sessions' : '$env:TEMP\\cognios-tmux';
  ...
  if (tmuxArgs.includes('capture-pane')) {
    ...
    const ps = host
      ...
      : `Get-Content (Join-Path $env:TEMP 'cognios-tmux\\${sid}.log') -Tail ${lines} -ErrorAction SilentlyContinue`;
  ...
  if (tmuxArgs.includes('has-session')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'cognios-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Get-Process -Id $p -ErrorAction SilentlyContinue | Out-Null; if ($?) { exit 0 } else { exit 1 } } else { exit 1 }`;
    return _winPowerShellCmd(task, ps);
  }
  ...
  if (tmuxArgs.includes('send-keys') && tmuxArgs.includes('C-c')) {
    const ps = host
      ...
      : `$p = Get-Content (Join-Path $env:TEMP 'cognios-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p) { Stop-Process -Id $p -ErrorAction SilentlyContinue }`;
    return _winPowerShellCmd(task, ps);
  }
...
function _winSessionStopTreePs(task) {
  ...
  return host
    ...
    : `${stopTree}; $p = Get-Content (Join-Path $env:TEMP 'cognios-tmux\\${sid}.pid') -ErrorAction SilentlyContinue; if ($p -match '^\\d+$') { Stop-Tree ([int]$p) }; Remove-Item (Join-Path $env:TEMP 'cognios-tmux\\${sid}.*') -Force -ErrorAction SilentlyContinue`;
}
  ...
  if (task.payload) {
    if (task.type === 'serve' && task.payload._cmd) {
      ...
      _updateTask(task.sessionId, {
        status: 'running',
        output: `${task.output || ''}\n\n[cognios] Retrying download. Progress may briefly look like a fresh download while HuggingFace checks cached/incomplete files; cached partial files will be reused when available.`.trim(),
        _retrying: true,
      });
      ...
      menuBtn.addEventListener('click', (e) => {
        ...
        if (_isWindows(task)) {
          ...
          const logCmd = host
            ...
            : `powershell -Command "Get-Content (Join-Path $env:TEMP 'cognios-tmux\\${task.sessionId}.log') -Wait"`;
...
const BG_LEADER_KEY = 'cognios-cookbook-bg-leader';
...
function _foregroundChatBusy() {
  try {
    return !!window.__cogniosChatBusy || Date.now() < (window.__cogniosChatBusyUntil || 0);
  } catch {
    return false;
  }
}
