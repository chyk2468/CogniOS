// ============================================
// CogniOS UI — Main Application Orchestrator
...
function _isForegroundChatBusy() {
  ...
  return !!window.__cogniosChatBusy
    || Date.now() < (window.__cogniosChatBusyUntil || 0)
...
function _submitMobileQueuedInput(input) {
  ...
  window.__cogniosQueueStreamingSubmit = now;
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__cogniosDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'CogniOS Chat';
  ...
  if (_opener) window._cogniosRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('cognios-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  (function initModeToggle() {
    ...
    window.__cogniosSetChatMode = setMode;
  ...
  const SPLASH_COUNT_KEY = 'cognios-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_COMPACT_WIDTH ? 'Message...' : 'Message CogniOS...');
      }
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      ...
      if (chk.checked) {
        ...
        if (typeof window.__cogniosSetChatMode === 'function') {
          window.__cogniosSetChatMode('chat');
        } else {
        ...
        if (typeof window.__cogniosSetChatMode === 'function') {
          window.__cogniosSetChatMode(_restoreMode === 'chat' ? 'chat' : 'agent');
        }
  ...
  const UI_VIS_KEY = 'cognios-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'cognios-toolbar-visibility';
    try {
  ...
  if (textarea) {
    ...
    window.addEventListener('cognios:chat-busy-change', () => _syncMobileEnterKeyHint(textarea));
      ...
      if (_shouldQueueFromMobileEnter(e, textarea) || (e.key === 'Enter' && !e.shiftKey && !e.isComposing && !isMobile)) {
        ...
        if (form) {
          if (_isForegroundChatBusy() && textarea.value && textarea.value.trim()) {
            ...
            window.__cogniosQueueStreamingSubmit = Date.now();
          }
          _submitChatFormDirect(form);
        }
      }
...
// ============================================
function startCogniOSApp() {
  if (window.__cogniosAppStarted) return;
  window.__cogniosAppStarted = true;
  const _bumpChatPriority = (ms = 10000) => {
    try {
      window.__cogniosChatBusyUntil = Math.max(window.__cogniosChatBusyUntil || 0, Date.now() + ms);
    } catch (_) {}
  };
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('cognios-doc-open-' + _curSession) === '1') {
  ...
  window._cogniosBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      if (sendBtn.dataset.mode === 'streaming') {
        if (hasText) window.__cogniosQueueStreamingSubmit = Date.now();
        handleSubmit(e);
        return;
      }
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._cogniosRouteOpener) {
          try { window._cogniosRouteOpener(); } catch (_) {}
          window._cogniosRouteOpener = null;
        }
      });
  } else {
  ...
  const runNonCriticalStartup = (fn, delay = 4000) => {
    let tries = 0;
    const run = () => {
      const busy = !!window.__cogniosChatBusy
        || Date.now() < (window.__cogniosChatBusyUntil || 0)
        || !!document.querySelector('.send-btn[data-mode="streaming"], .send-btn.send-pending');
      if (busy && tries < 12) {
        tries += 1;
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startCogniOSApp, { once: true });
} else {
  startCogniOSApp();
}
