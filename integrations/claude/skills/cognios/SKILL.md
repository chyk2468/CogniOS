---
name: cognios
description: Use when the user asks Claude Code to read or write CogniOS data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Claude Agent API. Requires COGNIOS_URL and COGNIOS_API_TOKEN.
---

# CogniOS

Use this skill when a user asks to interact with CogniOS from Claude Code.

## Configuration

Expect these environment variables:

- `COGNIOS_URL`: Base URL for the user's CogniOS instance, for example `http://127.0.0.1:7000`.
- `COGNIOS_API_TOKEN`: Scoped API token created in CogniOS Settings > Integrations > Add Integration > Claude Agent.

If either value is missing, do not guess credentials. Tell the user to create a Claude Agent token in CogniOS Settings and expose both values to the terminal session.
...
- **Calendar event ("meeting at 3pm", "dentist Tuesday 10am")** → calendar event. Use for scheduled time blocks, meetings, appointments, recurring schedules. These show up on the calendar grid; reminders for them are configured separately in CogniOS settings.
...
- All CogniOS data access MUST go through the scoped HTTP API under `/api/codex/*` (the canonical scope-gated agent API, shared by all agent integrations).
...
- Do not use SSH, Docker, direct Python imports, SQLite queries, MCP internals, browser cookies, or local files to read/write CogniOS user data.
...
```bash
python3 ~/.claude/skills/cognios/scripts/cognios_api.py capabilities
python3 ~/.claude/skills/cognios/scripts/cognios_api.py todos list
python3 ~/.claude/skills/cognios/scripts/cognios_api.py todos add "Follow up"
```
...
```bash
python3 ~/.claude/skills/cognios/scripts/cognios_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 ~/.claude/skills/cognios/scripts/cognios_api.py emails list 5
python3 ~/.claude/skills/cognios/scripts/cognios_api.py emails read UID
```
...
```bash
python3 ~/.claude/skills/cognios/scripts/cognios_api.py GET /api/codex/memory
python3 ~/.claude/skills/cognios/scripts/cognios_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
- Prefer `POST /api/codex/emails/draft-document` for agent-written email replies. It creates an editable CogniOS Document with `language: "email"` and does not touch IMAP/send.
...
python3 ~/.claude/skills/cognios/scripts/cognios_api.py cookbook tasks

# Tail the failing one (sessionId from `cookbook tasks`)
python3 ~/.claude/skills/cognios/scripts/cognios_api.py cookbook output serve-abc12345 400

# Stop the previous attempt before you try a new flag set
python3 ~/.claude/skills/cognios/scripts/cognios_api.py cookbook stop serve-abc12345

# Relaunch with new flags. cmd MUST begin with one of the allowlisted binaries.
python3 ~/.claude/skills/cognios/scripts/cognios_api.py cookbook serve \
...
If you are about to reach the CogniOS host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass CogniOS Settings and token scopes. Ask the user to enable the relevant Claude Agent tool toggle instead.
