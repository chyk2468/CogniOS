---
name: cognios
description: Use when the user asks Codex to read or write CogniOS data (todos, email, calendar, memory, documents) or to launch/monitor/stop a Cookbook model-serve task through the scoped Codex Agent API. Requires COGNIOS_URL and COGNIOS_API_TOKEN.
---

# CogniOS

Use this skill when a user asks to interact with CogniOS from Codex.
...
- `COGNIOS_API_TOKEN`: Scoped API token created in CogniOS Settings > Integrations > Add Integration > Codex Agent.

If either value is missing, do not guess credentials. Tell the user to create a Codex Agent token in CogniOS Settings and expose both values to the terminal session.
...
- All CogniOS data access MUST go through the scoped HTTP API under `/api/codex/*`.
...
```bash
python3 integrations/codex/scripts/cognios_api.py capabilities
python3 integrations/codex/scripts/cognios_api.py todos list
python3 integrations/codex/scripts/cognios_api.py todos add "Follow up"
```
...
```bash
python3 integrations/codex/scripts/cognios_api.py POST /api/codex/todos '{"action":"add","title":"Call dentist","due_date":"tomorrow at 5pm"}'
```
...
```bash
python3 integrations/codex/scripts/cognios_api.py emails list 5
python3 integrations/codex/scripts/cognios_api.py emails read UID
```
...
```bash
python3 integrations/codex/scripts/cognios_api.py GET /api/codex/memory
python3 integrations/codex/scripts/cognios_api.py POST /api/codex/memory '{"text":"User prefers SI units","category":"preference"}'
```
...
- Prefer `POST /api/codex/emails/draft-document` for Codex-written email replies. It creates an editable CogniOS Document with `language: "email"` and does not touch IMAP/send.
...
```bash
python3 ~/plugins/cognios/scripts/cognios_api.py cookbook tasks
python3 ~/plugins/cognios/scripts/cognios_api.py cookbook output serve-abc12345 400
python3 ~/plugins/cognios/scripts/cognios_api.py cookbook stop serve-abc12345
python3 ~/plugins/cognios/scripts/cognios_api.py cookbook serve \
...
If you are about to reach the CogniOS host/container, import app internals, query the database, or call MCP helper modules directly, stop. Those paths bypass CogniOS Settings and token scopes. Ask the user to enable the relevant Codex Agent tool toggle instead.
