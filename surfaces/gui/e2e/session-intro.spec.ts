...
test("three rows, no Set-me-up; gated rows show Configure › and expand the rail's Access section", async ({
  ...
  await expect(page.getByPlaceholder(/Ask CogniWork/)).toHaveValue("");
});
...
test("ready rows reveal Start → on hover and prefill the composer", async ({ page }) => {
  ...
  await hs.click();
  await expect(page.getByPlaceholder(/Ask CogniWork/)).toHaveValue(/HubSpot leads/);
  ...
  await gh.click();
  await expect(page.getByPlaceholder(/Ask CogniWork/)).toHaveValue(/weekly progress report/);
});
