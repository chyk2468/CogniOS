...
test("signed out: the account row is the sign-in home; managed connector still connects manually", async ({
  ...
  await expect(menu).toContainText("one-click connections need CogniOS Cloud");
  ...
  await expect(modal.getByTestId("managed-connect")).toContainText("Sign in to CogniOS Cloud");
...
test("signed in: account row shows the name; one-click appears; sign out from the menu", async ({
  ...
  await page.getByTestId("account-row").click();
  const menu = page.getByTestId("account-menu");
  await expect(menu).toContainText("rohit@cognios.com");
