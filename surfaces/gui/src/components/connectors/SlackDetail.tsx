          ...
          <div className="text-[12.5px] text-muted flex items-center gap-1.5">
            {c.connected ? (
              <>
                <span
                  className={
                    "w-2 h-2 rounded-md " + (relay ? relayHealth(slack).dot : "bg-ok")
                  }
                />
    ...
    <div className={ROW}>
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        ...
        {allowed.map((u) => (
          <span
            key={u}
            className="inline-flex items-center gap-1.5 pl-1 pr-2 py-0.5 rounded-md bg-bg border border-border text-[12.5px]"
            title={`id ${u}`}
            data-testid={u === installerId ? "people-chip-you" : undefined}
          >
            <span className="w-5 h-5 rounded-md bg-accentSoft text-accent grid place-items-center text-[9px] font-bold">
              {initials(label(u))}
            </span>
            {label(u)}
...
function PersonPicker({
  ...
  return (
    <span className="relative" ref={wrap}>
      <button
        ref={btn}
        className="inline-flex items-center px-2 py-0.5 rounded-md border border-dashed border-border text-[12.5px] text-muted hover:text-fg hover:border-faint"
      ...
      {open && (
        <div
          className="fixed z-50 w-72 rounded-md border border-border bg-panel shadow-lg p-1"
          style={{ top: pos?.top, left: pos?.left }}
          data-testid="person-picker"
        >
          <input
            autoFocus
            className="w-full bg-bg border border-border rounded-md px-2 py-1 text-[12.5px] outline-none placeholder:text-faint"
          ...
          <div className="max-h-56 overflow-y-auto py-1">
            {err ? (
              ...
              candidates.map((m) => (
                <button
                  key={m.id}
                  className="block w-full text-left px-2 py-1.5 rounded-md hover:bg-bg"
