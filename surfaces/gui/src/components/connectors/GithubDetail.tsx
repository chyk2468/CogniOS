          ...
          <div className="text-[12.5px] text-muted flex items-center gap-1.5">
            {c.connected ? (
              <>
                <span
                  className={
                    "w-2 h-2 rounded-md " + (relay ? relayHealth(status).dot : "bg-ok")
                  }
                />
    ...
    <div className={ROW}>
      ...
      <span className="min-w-0 flex-1 flex flex-wrap items-center gap-1.5">
        ...
        {allowed.map((login) => (
          <span
            key={login}
            className="inline-flex items-center gap-1.5 pl-2 pr-2 py-0.5 rounded-md bg-bg border border-border text-[12.5px]"
          >
