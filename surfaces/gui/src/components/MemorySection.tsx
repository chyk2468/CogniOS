  ...
  return (
    <section>
      ...
      <div className={CARD + " p-4 mb-4"} data-testid="memory-toggle-card">
        ...
        {toggleMsg && (
          <div className="text-[12.5px] text-muted mt-3 pt-3 border-t border-border" data-testid="memory-toggle-msg">
            {toggleMsg}
          </div>
        )}
      </div>
  ...
  return (
    <div className={CARD + " p-4"} data-testid="user-rules-card">
      ...
      <textarea
        ...
        className="w-full mt-2.5 px-3 py-2.5 rounded-md border border-border bg-bg text-[13px] text-fg outline-none focus:border-accent resize-y leading-relaxed"
      />
...
function MemoryRow({ entry, onChanged }: { entry: MemoryEntry; onChanged: () => void }) {
    ...
    return (
      <div className="py-2.5" data-testid={`memory-edit-${entry.id}`}>
        <textarea
          ...
          rows={2}
          autoFocus
          className="w-full px-3 py-2 rounded-md border border-border bg-bg text-[13px] text-fg outline-none focus:border-accent resize-y leading-relaxed"
          onKeyDown={(e) => {
        ...
        <div className="flex items-center gap-2.5 mt-1.5">
          ...
          <button className="text-[12.5px] text-muted hover:text-fg" onClick={() => setEditing(false)}>
            cancel
          </button>
        </div>
      </div>
  ...
  return (
    <div className="py-2.5 flex items-start gap-2.5 group" data-testid={`memory-row-${entry.id}`}>
      ...
      <button
        className="text-faint hover:text-fg shrink-0 mt-0.5"
        title="Fix this"
