...
const CARD = "rounded-md2 border border-border bg-panel";
const FIELD_LABEL = "text-[12.5px] font-medium text-fg";
const INPUT =
  "w-full min-w-0 px-3 py-2 rounded-md border border-border bg-bg text-[13px] text-fg outline-none focus:border-accent";
const BTN_ACCENT =
  "text-[12.5px] px-3 py-2 rounded-md bg-accent text-white shrink-0 disabled:opacity-40";
const BTN_BORDERED =
  "text-[12.5px] px-3 py-2 rounded-md border border-border bg-bg hover:border-border shrink-0";
const BADGE =
  "text-[11px] px-2 py-0.5 rounded-md border border-border bg-bg text-muted shrink-0";
    ...
    <section>
        ...
        <div className="relative shrink-0">
          ...
          {addOpen ? (
            <>
              ...
              <div
                role="menu"
                className="absolute right-0 top-full mt-1.5 w-80 rounded-md2 border border-border bg-panel shadow-xl z-20 p-1.5"
                onKeyDown={(e) => e.key === "Escape" && setAddOpen(false)}
              >
                <button
                  role="menuitem"
                  className="w-full text-left px-3 py-2 rounded-md hover:bg-bg"
                  onClick={() => {
                    setAddOpen(false);
                ...
                <button
                  role="menuitem"
                  className="w-full text-left px-3 py-2 rounded-md hover:bg-bg disabled:opacity-40"
                ...
                >
                  <div className="text-[13px] font-medium">Create with CogniOS</div>
                  <div className="text-[11.5px] text-muted">
      ...
      {notice ? (
        <div
          role="status"
          className={
            "mb-3 flex items-start gap-2 rounded-md border px-3 py-2 text-[12.5px] " +
            (notice.tone === "ok"
      ...
      {upload ? (
        <div className={`${CARD} p-4 mb-4`}>
          ...
          <pre className="text-[12px] bg-bg border border-border rounded-md p-3 whitespace-pre-wrap max-h-64 overflow-y-auto mb-2">
            {upload.instructions}
          </pre>
          {upload.files?.length ? (
          ...
          <div key={row.name} className="flex items-center gap-3 px-4 py-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {row.files ? (
                  <button
                    className="inline-flex items-center gap-1 text-[11px] px-1.5 py-0.5 rounded-md border border-border bg-bg text-muted hover:text-fg hover:border-border shrink-0"
                    title="Show folder"
