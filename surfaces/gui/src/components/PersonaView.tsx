...
const TAG_CORE =
  "text-[10px] px-1.5 py-0.5 rounded-md bg-warnSoft/70 text-warnInk border border-warnInk/15";
const TAG_MCP = "text-[10px] px-1.5 py-0.5 rounded border border-border text-faint";
const BTN_ACCENT = "text-[12px] px-2.5 py-1.5 rounded-md bg-accent text-white shrink-0";
const BTN_BORDERED =
  "text-[12px] px-2.5 py-1.5 rounded-md border border-border bg-bg hover:border-border shrink-0";
...
export function PersonaView({
  ...
  const header = (
    <div className="h-12 shrink-0 px-5 flex items-center gap-3 border-b border-border bg-bg">
      {onBack && (
        <>
          <button
      ...
      <div className="flex-1 overflow-y-auto hairline-scroll">
        <div className="max-w-3xl mx-auto px-7 py-6 space-y-6">
          ...
          <header className="flex items-start gap-3.5">
            <span className="w-12 h-12 rounded-md2 bg-panel border border-border grid place-items-center text-[22px]">
          ...
          {detail.description && (
            <section>
              ...
              <p className="text-[14px] leading-relaxed text-fg/90">{detail.description}</p>
            </section>
          )}
          ...
          {detail.tools.length > 0 && (
            <section>
              ...
              <div className="flex flex-wrap gap-1.5">
                {detail.tools.map((t) => (
                  <span
                    className="px-2 py-1 rounded-md bg-panel border border-border text-[12px] font-mono"
                    key={t}
                  >
                    {t}
                  </span>
          ...
          {detail.recommends.length > 0 && (
            <section>
              ...
              <div className="rounded-md2 border border-border overflow-hidden">
                {detail.recommends.map((r, i) => {
                  const isMcp = r.kind === "mcp";
                  return (
                    <div
                      className={
                        "flex items-center gap-3 p-3 bg-panel" + (i > 0 ? " border-t border-border" : "")
                      }
                      key={`${r.kind}:${r.ref}`}
                    >
                      ...
                      {r.connected ? (
                        <span className="inline-flex items-center gap-1 text-[11.5px] text-ok shrink-0">
                          <span className="w-1.5 h-1.5 rounded-md bg-ok" />
                          connected
                        </span>
                      ) : (
                        <button
            ...
            <section>
              ...
              <div className="space-y-1.5">
                {detail.default_connections.map((c) => (
                  <div
                    className={
                      "flex items-center gap-3 p-2.5 rounded-md2 border border-border bg-panel" +
