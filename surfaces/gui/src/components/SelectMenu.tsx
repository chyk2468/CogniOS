...
export function SelectMenu({
  ...
  return (
    <div className="relative">
      <button
        type="button"
        className="w-full flex items-center gap-2 px-2.5 py-2 rounded-md border border-border bg-bg text-[13px] text-fg hover:border-border"
      ...
      >
        ...
        {current?.dot && <span className="w-1.5 h-1.5 rounded-md bg-ok shrink-0" />}
      ...
      {open && (
        <>
          ...
          <div
            role="listbox"
            aria-label={ariaLabel}
            className="absolute z-40 left-0 right-0 mt-1 max-h-72 overflow-y-auto rounded-md border border-border bg-panel shadow-xl p-1"
          >
            {options.map((o, i) => {
                ...
                o.group && o.group !== options[i - 1]?.group ? (
                  <div
                    className={
                      ...
                      (i === 0 ? "pt-1" : "pt-2.5 mt-1.5 border-t border-border")
                    }
                  >
                    {o.group}
                  </div>
              ...
              return (
                ...
                >
                  <span className="min-w-0 flex-1">
                    <span
                      className={
                        "block text-[13px] truncate " + (sel ? "font-semibold text-fg" : "text-fg")
                      }
                    >
                      {o.label}
                    </span>
                  ...
                  <span
                    className={
                      "w-1.5 h-1.5 rounded-md shrink-0 " + (o.dot ? "bg-ok" : "bg-transparent")
                    }
