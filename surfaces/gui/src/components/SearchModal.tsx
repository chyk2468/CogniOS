...
export function SearchModal({
    ...
    return (
      <button
        ...
        className={
          ...
          (isActive ? "bg-accentSoft" : "hover:bg-bg")
        }
      ...
      >
        ...
        {idx < 9 && (
          <kbd className="text-[10.5px] text-faint bg-bg border border-border rounded px-1.5 py-0.5 shrink-0 font-sans">
            ⌘{idx + 1}
          </kbd>
        )}
      </button>
  ...
  return (
    <div className="fixed inset-0 z-50" onKeyDown={onKey}>
      ...
      <div className="absolute left-1/2 top-[14vh] -translate-x-1/2 w-[640px] max-w-[92vw] rounded-md2 border border-border bg-panel shadow-2xl overflow-hidden">
        <div className="px-4 pt-3.5 pb-2.5 border-b border-border flex items-center gap-2.5">
          ...
          <input
            ...
            className="flex-1 bg-transparent outline-none text-[15px] text-fg placeholder:text-faint"
          />
          <kbd className="text-[10.5px] text-faint bg-bg border border-border rounded px-1.5 py-0.5 font-sans">
            Esc
          </kbd>
        </div>
