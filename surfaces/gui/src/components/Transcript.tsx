...
function approvalChip(resolved: ApprovalDecision | undefined) {
  if (resolved === "deny")
    return <span className="text-[10.5px] px-1.5 rounded-md bg-dangerSoft text-danger shrink-0">✕ declined</span>;
  return (
    <span
      className="text-[10.5px] px-1.5 rounded-md bg-okSoft text-ok shrink-0"
...
function LineText({ line }: { line: HumanLine }) {
  return (
    <span className="min-w-0 text-[13px] leading-relaxed">
      ...
      {line.obj && <span className="text-fg">{line.obj}</span>}
...
function StepRow({ tool, approval }: { tool: ToolItem; approval?: ApprovalItem }) {
  ...
  return (
    <div>
      <div className="group flex items-baseline gap-2 px-2 py-0.5 rounded-md hover:bg-bg" data-testid="turn-step">
        ...
        {!!tool.standingRule && (
          <span
            className="text-[10.5px] px-1.5 rounded-md bg-tealSoft text-tealInk shrink-0"
            data-testid="tool-standing-rule"
            title={`Auto-allowed by standing approval: ${tool.standingRule}.`}
          >
            auto-allowed
          </span>
        )}
      ...
      {raw && (
        <pre className="ml-8 mr-2 my-1 px-2.5 py-1.5 rounded-md border border-border bg-bg font-mono text-[11.5px] leading-relaxed text-muted whitespace-pre-wrap break-words max-h-56 overflow-auto">
...
function TurnGroup({
  ...
  return (
    <details className="stepgroup" open={open}>
      ...
      {open && (
        <div className="ml-1.5 mt-1 pl-2 border-l-2 border-border flex flex-col gap-0.5">
          {rows.map((row, i) =>
            row.type === "narr" ? (
    ...
    <div className="transcript">
      {blocks.map((block, bi) => {
        ...
        switch (item.kind) {
          ...
          case "user":
            return (
              <div className="group self-end max-w-[82%] flex flex-col items-end" key={bi}>
                <div className="bubble-user px-3.5 py-2.5 rounded-md border border-border bg-panel text-fg text-[14px] leading-relaxed whitespace-pre-wrap font-mono shadow-sm">
                  {item.attachments && item.attachments.length > 0 && (
                    <div className="bubble-attachments mb-2 flex flex-wrap gap-2">
                      {item.attachments.map((a, i) =>
                        a.kind === "image" ? (
                          <img key={i} className="msg-img max-h-48 rounded border border-border" src={a.data_url} alt={a.name} />
                        ) : (
                          <span key={i} className="msg-file text-[12px] px-2 py-1 rounded bg-bg border border-border font-mono">📄 {a.name}</span>
                        ),
                      )}
                    </div>
                  )}
