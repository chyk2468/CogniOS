...
const SELECT = "px-2.5 py-2 rounded-md border border-border bg-bg text-[13px] text-fg shrink-0";
const INPUT =
  "flex-1 min-w-0 px-3 py-2 rounded-md border border-border bg-bg text-[13px] text-fg outline-none focus:border-accent";
const BTN_ACCENT = "text-[12.5px] px-3 py-2 rounded-md bg-accent text-white shrink-0 disabled:opacity-40";
const BTN_BORDERED =
  "text-[12.5px] px-2.5 py-1.5 rounded-md border border-border bg-bg hover:border-border shrink-0 disabled:opacity-40 disabled:hover:border-border";
          ...
          <div key={p.id} className="px-4 py-3">
            ...
            {!p.builtin &&
              (confirmDel === p.id ? (
                <span className="flex items-center gap-1.5 shrink-0">
                  <button
                    className="text-[12px] px-2 py-1.5 rounded-md bg-danger text-white"
            ...
            {confirmOff === p.id && (
              ...
              >
                ...
                <button
                  className="text-[12px] px-2.5 py-1.5 rounded-md bg-accent text-white shrink-0"
        ...
        <div className="mt-4 space-y-2">
          {consent.map((c) => (
            <div key={c.id} className={CARD + " p-3.5"}>
              ...
              <div className="text-[12px] text-fg">Tools: {c.tools.join(", ") || "—"}</div>
