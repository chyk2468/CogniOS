...
const BTN_PRIMARY =
  "px-3 py-1.5 rounded-md bg-accent text-white text-[12.5px] font-medium hover:brightness-105 disabled:opacity-40 disabled:hover:brightness-100";
const BTN_BORDERED =
  "px-3 py-1.5 rounded-md border border-border bg-bg text-[12.5px] hover:border-border";
// §35 approval buttons: blue border for the primary, quiet Deny (matches ApprovalCard).
const BTN_ACCENT =
  "px-3 py-1.5 rounded-md border border-accent text-accent text-[12.5px] font-semibold hover:bg-accentSoft";
const BTN_QUIET = "px-3 py-1.5 text-[12.5px] text-faint hover:text-danger";
const OPT_BASE =
  "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border text-[13px] transition-colors";
const OPT_OFF = "border-border bg-bg text-fg hover:border-accent hover:bg-accentSoft/50";
const OPT_ON = "border-accent bg-accentSoft text-accent font-medium";
const INPUT =
  "flex-1 min-w-0 rounded-md bg-bg border border-border px-3 py-2 text-[13px] text-fg placeholder:text-faint outline-none focus:border-border";
// Rich options stack as full-width rows (pills can't hold a description line).
const ROW_BASE = "w-full text-left rounded-md border px-3 py-2 transition-colors";
const ROW_OFF = "border-border bg-bg hover:border-accent hover:bg-accentSoft/50";
...
function QuestionBlock({ spec, onAnswer }: { spec: QSpec; onAnswer: (a: string) => void }) {
  ...
  const recommendedTag = (
    <span className="text-[10px] uppercase tracking-[0.04em] font-semibold text-ok bg-okSoft border border-okLine rounded-md px-1.5 py-px shrink-0">
      Recommended
    </span>
  );
      ...
      {options.map((o, i) => {
        ...
        return (
          ...
          >
            <span
              className={
                "flex items-center gap-2 text-[13px] " + (on ? "text-accent font-medium" : "text-fg font-medium")
              }
            >
        ...
        (hasPreview ? (
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-2.5">
            ...
            <pre
              ...
              className="flex-1 min-w-0 rounded-md border border-border bg-bg p-3 text-[12px] leading-relaxed font-mono whitespace-pre overflow-auto max-h-72 text-fg"
            >
              {preview}
            </pre>
          </div>
  ...
  return (
    <>
      ...
      <div className={SEC + " flex items-center gap-1.5"} data-testid={grouped ? "question-stepper" : undefined}>
        {grouped && step > 0 && (
          <button
            className="text-faint hover:text-fg leading-none text-[13px]"
...
export function InboxItemCard({
  ...
  return (
    <div
      className={
        compact
          ? "max-w-3xl mx-auto mb-2.5 rounded-md2 border border-border bg-panel px-4 py-3.5 shadow-[0_8px_24px_rgba(0,0,0,0.08)]"
          : "mb-2.5 rounded-md2 border border-border bg-panel px-3.5 py-3"
      }
    >
