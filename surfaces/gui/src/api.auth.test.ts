...
it("authenticates REST and session WebSocket calls with the launch token", async () => {
  vi.stubGlobal("__COGNIWORK_API_TOKEN__", "launch-token");
  const request = vi.fn(async (_url: string, init?: RequestInit) => {
    expect(new Headers(init?.headers).get("X-CogniOS-Token")).toBe("launch-token");
  ...
  expect(socket.protocols).toEqual(["cognios", "launch-token"]);
});
