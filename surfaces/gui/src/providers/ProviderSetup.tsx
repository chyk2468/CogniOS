...
export function ProviderMark({ name, title, size = 32 }: { name: string; title: string; size?: number }) {
  ...
  return (
    <span
      className="rounded-md border border-border grid place-items-center shrink-0"
...
export function ProviderCards({
  ...
  const card =
    "flex items-center gap-2.5 rounded-md border border-border bg-panel px-3 py-2.5 text-left hover:border-border transition-colors";
  return (
...
export function ProviderForm({
  ...
  const input =
    "w-full px-3 py-2 rounded-md border bg-panel text-[13.5px] outline-none focus:border-accent";
    ...
    <div key={f.key}>
      ...
      <div className="flex gap-2">
        <div className="relative flex-1 min-w-0">
          <input
            className={input + (ps.savedState && testable ? " border-ok pr-32" : " border-border")}
          ...
          {ps.fieldSaved === f.key && (
            <span
              className="absolute right-2 top-1/2 -translate-y-1/2 text-[11px] font-medium text-ok bg-okSoft rounded-md px-2 py-0.5 pointer-events-none"
        ...
        {testable && (
          <button
            className="px-4 rounded-md border border-border text-[13px] font-medium text-fg hover:border-border shrink-0 disabled:opacity-40"
  ...
  return (
    <div>
      <button className="text-[12.5px] text-muted hover:text-fg" onClick={ps.backToGallery} data-testid={`${tp}-back`}>
        ‹ All providers
      </button>
      ...
      {choice && (
        <div>
          ...
          <div
            className="inline-flex gap-0.5 rounded-[10px] border border-border bg-line/40 p-[3px]"
            role="radiogroup"
            ...
            {(choice.choices || []).map((c) => {
              ...
              return (
                <button
                  ...
                  aria-checked={active}
                  className={
                    "flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[12.5px] whitespace-nowrap transition-colors " +
                    (active
                      ? "bg-panel text-fg font-medium shadow-sm ring-1 ring-line"
                ...
                >
                  {c.label}
                  {c.tag && (
                    <span className="text-[9.5px] font-semibold uppercase tracking-wide text-accent bg-accentSoft rounded-md px-1.5 py-px">
                      {c.tag}
                    </span>
                  )}
                </button>
          ...
          <div className="mt-2.5 rounded-md border border-border bg-bg/60 px-4 pb-3.5 pt-3">
            {selected?.desc && <p className="text-[12px] text-muted">{selected.desc}</p>}
            {selected?.command && (
              <button
                className="mt-2.5 inline-flex items-center gap-2 rounded-md border border-border bg-panel px-2.5 py-1.5 text-[12px] font-mono text-fg hover:border-border"
            ...
            <div className="mt-3.5 flex items-center justify-between gap-3 border-t border-border pt-3">
              ...
              <button
                className="shrink-0 rounded-md border border-accent bg-accent px-4 py-1.5 text-[13px] font-medium text-white hover:brightness-105 disabled:opacity-40"
                onClick={() => ps.runTestAndSave()}
      ...
      {(() => {
        ...
        if (!ps.showEndpoint)
          return (
            <button
              className="block self-start text-[12.5px] text-muted hover:text-fg mt-4"
