...
CogniOS keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/cognios-backup` tool
...
```bash
# Create a snapshot → backups/cognios-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/cognios-backup snapshot

# List existing snapshots (most recent first)
./scripts/cognios-backup list

# Check a tarball's integrity without extracting it
./scripts/cognios-backup verify backups/cognios-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/cognios-backup restore backups/cognios-backup-20260101-120000.tar.gz --yes
```
...
./scripts/cognios-backup snapshot --out /mnt/nas/cognios-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/cognios-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/cognios && ./scripts/cognios-backup snapshot --out "/mnt/nas/cognios-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `cognios-backup` therefore does not capture the Docker
