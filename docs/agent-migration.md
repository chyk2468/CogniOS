...
CogniOS should be able to learn from another agent without blindly trusting
...
Markdown notes, or any other agent can have its own adapter, but CogniOS only
...
- **Memory candidates** are short facts or preferences that can be reviewed
  before being saved into CogniOS memory.

This keeps CogniOS' existing memory-review flow intact while giving it better
...
The helper does not write to `data/`, call an LLM, import CogniOS modules, or
...
A future CogniOS importer should treat the manifest as untrusted user-provided
...
Nonstandard folders should be adapter details, not required CogniOS concepts.
