...
This populates the `demo@cognios.local` Dovecot account (which has NO mbsync
...
USER = os.getenv("DEMO_IMAP_USER", "demo@cognios.local")
...
LOOKUP_MSGID = "<demo-lookup-deepseek@cognios.local>"
...
def _msg(*, frm, to=None, subject, text, html=None, days_ago=0, hours_ago=0,
    ...
    m["Message-ID"] = msg_id or make_msgid(domain="cognios.local")
    if in_reply_to:
...
def build_dataset() -> list[dict]:
    ...
    add("INBOX", "", _msg(
        ...
        text=("Hey,\n\nSaw your work on the CogniOS stack — seriously impressive. "
        ...
        html=("<p>Hey,</p><p>Saw your work on the <b>CogniOS</b> stack — seriously "
    ...
    add("INBOX", "", _msg(
        frm="Greg <greg@cognios-demo.example>",
    ...
    orig_id = make_msgid(domain="cognios.local")
    add("Sent", "(\\Seen)", _msg(
    ...
    add("INBOX", "(\\Flagged)", _msg(
        frm="Ops Bot <ops@cognios-demo.example>",
        subject="[URGENT] prod is on fire 🔥 — cognios-ui 502s",
        days_ago=0, hours_ago=1,
        text=("PAGE: cognios-ui is returning 502s on the /api/chat endpoint.\n"
...
def _seed_cache() -> None:
    ...
    reply = (
        ...
        "— drafted for you by your CogniOS assistant"
    )
    ...
    try:
        ...
        con.execute(
            ...
            (LOOKUP_MSGID, "", "INBOX", "quick q for the slide — DeepSeek-V3 param count?",
             "greg@cognios-demo.example", summary, "demo", now))
        con.commit()
...
def _wipe(conn: imaplib.IMAP4) -> int:
    ...
    """
    safe_target = USER.endswith("@cognios.local") or HOST in ("localhost", "127.0.0.1", "::1")
...
def main() -> int:
    ...
    try:
        ...
        print(f"seeded {len(items)} demo message(s) into {USER} "
              f"(INBOX + Sent). Switch to the 'Demo' account in CogniOS to view.")
        return 0
    finally:
        try:
            conn.logout()
