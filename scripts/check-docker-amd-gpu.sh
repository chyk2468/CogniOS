...
# see /dev/kfd and /dev/dri. The CogniOS slim image does not include ROCm tools
...
RENDER_GID=""
VIDEO_GID=""
TEST_IMAGE="${COGNIOS_AMD_TEST_IMAGE:-alpine:3.20}"
...
Environment:
  COGNIOS_AMD_TEST_IMAGE   Docker image for the passthrough smoke
                            (default: alpine:3.20)
USAGE
}
...
_print_next_steps() {
    ...
    echo
    echo "After restarting CogniOS, verify the slim app container sees devices:"
    echo "  docker compose exec cognios sh -lc 'test -e /dev/kfd && test -d /dev/dri && ls -l /dev/kfd /dev/dri/renderD*'"
    echo
    echo "Note: rocm-smi/rocminfo are not expected inside the slim CogniOS image."
...
echo "=== CogniOS AMD Docker GPU diagnostic ==="
echo
_check_host_devices
_check_groups
_check_host_rocm
