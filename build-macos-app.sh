#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for CogniOS.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/CogniOS.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/CogniOS.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with COGNIOS_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="CogniOS"
INSTALL_DIR="$REPO_DIR"
PORT="${COGNIOS_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
# ── Icon (best effort) — center-crop docs/cognios.jpg to a square .icns ──
if [ -f "$REPO_DIR/docs/cognios.jpg" ] && command -v sips >/dev/null 2>&1; then
  TMPIMG="$(mktemp -d)"
  ...
  sips -c 720 720 "$REPO_DIR/docs/cognios.jpg" --out "$TMPIMG/sq.png" >/dev/null 2>&1 || cp "$REPO_DIR/docs/cognios.jpg" "$TMPIMG/sq.png"
  sips -z 512 512 "$TMPIMG/sq.png" --out "$TMPIMG/icon.png" >/dev/null 2>&1
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/cognios.icns" >/dev/null 2>&1; then
    echo "  icon:        cognios.icns"
  else
    echo "  icon:        (skipped — conversion failed)"
  fi
  rm -rf "$TMPIMG"
else
  echo "  icon:        (skipped — no docs/cognios.jpg)"
fi
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.cognios.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>cognios</string>
...
#!/bin/bash
# CogniOS.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/cognios-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"CogniOS\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"CogniOS\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "CogniOS isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "CogniOS failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "CogniOS is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag CogniOS to Applications)"
